...
if __name__ == '__main__':
    ...
    files_ref = get_icar_filelist(ref_start.split('-')[0], ref_end.split('-')[0], dt=dt)  ### TEST 2024-04-29
    ...
    for t in range(ts,len(time_s)):
        ...
        print(" \n ")
        print("    - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ")
        print(f"    - - -  bias corrected file written to {path_out}/{model}_{scen}/icar_{dt}_livgrid_{model}_{scen.split('_')[0]}_{time_s[t].split('-')[0]}-{time_f[t].split('-')[0]}.nc   - - - ")
